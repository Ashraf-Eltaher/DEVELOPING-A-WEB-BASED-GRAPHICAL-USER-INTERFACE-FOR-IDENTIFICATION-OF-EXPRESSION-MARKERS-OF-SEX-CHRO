library(shiny)
library(shinythemes)
source("D:/MXP project/Datasets MXP3/Appcode.R")
                        ...
                        conditionalPanel("input.gender1 != ''",
                            selectizeInput(inputId = "gender2",
                                      ...
                                      options = list(
                                        ...
                                        onInitialize = I('function() { this.setValue(""); }')))),
                        # conditionalPanel("input.gender2 != ''",
                        #     selectizeInput(inputId = "gender3",
                        #                    label = NULL,
                        #                    choices = '',
                        #                    options = list(
                        #                      placeholder = 'Sample 3',
                        #                      onInitialize = I('function() { this.setValue(""); }')))),
                        # conditionalPanel("input.gender3 != ''",
                        #     selectizeInput(inputId = "gender4",
                        #                    label = NULL,
                        #                    choices = '',
                        #                    options = list(
                  ...
                  mainPanel(
                        tabsetPanel(
                          ...
                          tabPanel("Gene ontology",tableOutput("goTable"))
                                          # conditionalPanel("input.button != 0",
                                          #                  radioButtons('go','',choices = c('table','plot'), selected = 'table', inline=T),offset = 2),
                                          # conditionalPanel("input.go == 'table'",
                                          #                  tableOutput("goTable")),
                                          # conditionalPanel("input.go == 'plot'",
                                          #                  plotOutput("goPlot")))
                    )
                    )))
...
server <- function(input, output,session) {
  ...
  # observe({
  #   input$gender3
  #   updateSelectizeInput(session, 'gender4',choices = genders[!(genders %in% c(input$gender1,input$gender2,input$gender3))],options = list(
  #     placeholder = 'Sample 4',
  #     onInitialize = I('function() { this.setValue(""); }')))
  # })
  ...
  #   if(input$gender4 != ''){
  #     inputSamples <<- c(input$gender2,input$gender3,input$gender4)
  #   }else if(input$gender3 != ''){
  #     inputSamples <<- c(input$gender2,input$gender3)
  #   }else{
  #     inputSamples <<- input$gender2
  #   }
    ...
    # defitDF <- filter.fitDF(valuesDF,input$tableLogFC,input$tablePvalue)
    # deDF <- subset(valuesDF,rownames(valuesDF) %in% rownames(defitDF))                                      
    ...
    #output$goPlot  <- renderPlot({goPlotData(goData(valuesDF))})
    })
