...
exprs_data <- unique(read.csv("D:/MXP project/Datasets MXP3/Expression values.txt",sep="\t",header=TRUE,dec=","))  ## Reading expression data ##
pdata = read.csv("D:/MXP project/Datasets MXP3/phenoData.txt",sep="\t",header=TRUE) ## Reading phenoData ##
pdata = subset(pdata,Mutation != '2x' & Mutation != 'Unkown')
expr_annotated <- read.csv("D:/MXP project/Datasets MXP3/Expression values Annotated.txt",sep=",",header=TRUE,dec=",")  ## Reading annotated expression data for Manhattan plot##
...
#biPr <- "D:/MXP project/Datasets MXP3/c5.go.bp.v7.2.entrez.gmt"
#huPh <- "D:/MXP project/Datasets MXP3/c5.hpo.v7.2.entrez.gmt"
...
createValuesDF <- function(sample1,sample2,chromosome = 'all'){# chromosome -> to choose chrom. X,autosomal or all(default)
  ...
  sex <<- factor(groups, levels = sampleList) # Creating factor of sample types for PCA plot
...
createPCA <- function(dataframe){
  ...
  # Create PCA data
  fdata <- filter.fitDF(dataframe,pvalue.value = 0.05)
...
tsnePlot <- function(dataframe){
  ...
  # tsne_plot <- Rtsne::Rtsne(dataframe,perplexity=10, check_duplicates = FALSE)
  # plot(tsne_plot$Y,col = c(myColors[sampleList[1]],myColors[sampleList[2]]), bg= myColors, pch = 20, cex = 1)
}
                            #######################
...
heatmapData <- function(dataframe){
  ...
  defitDF <- filter.fitDF(dataframe,pvalue.value = 0.05)
...
manPlotData <- function(dataframe){
  ...
  rows <- subset(rows,Probe_Id %in% manTable$SNP)
  manTable <- cbind(manTable,rows[,c('Chromosome','BP')])
  ...
  manTable$P.Value = fitData(dataframe)$P.Value
  manTable <- manTable[!is.na(manTable$BP),]
  manTable <<- manTable[manTable$Chromosome != 0,]
  manhattan(manTable, chr="Chromosome", bp="BP", snp="SNP", p="P.Value",chrlabs =  c(1:22, "X", "Y", "MT"))
}

                    ###################################
## A function to filter tables' samples and volcano plot according to logFC & p value##
...
goData <- function(dataframe){
  fdat <- filter.fitDF(dataframe,pvalue.value =0.05)
...
# }
# 
# goPlotData <- function(godata){
#   gCol <- colMap(gstat$Significant)
#   plot(pValue.classic, pValue.elim, xlab = "p-value classic", ylab = "p-value elim",
#        pch = 19, cex = gSize, col = gCol)
#   SDMTools::legend.gradient(cbind(x =c(0.95,0.98,0.98,0.95), y =c(0,0.3,0.3,0)),
#                             cols = rev(heat.colors(100)), limits = c("Low","High"),
#                             title = '# DEG')
#   #par(mar=c(3,3,3,3))
#   # plotfunctions::gradientLegend(godata$Significant,rev(heat.colors(100)),
#   #                               inside = F,tick.col = NA,title = '# DEG')
# }
                    ###################################
